...
export async function GET(_req: NextRequest, { params }: Params): Promise<Response> {
  ...
  try {
    assertCapability(device.provider, "hasCamshot");
    const provider = getProvider(device.provider);
    const buffer = await provider.getCamshot(device);
    ...
    if (err instanceof ProviderError) {
      if (err.status === 404) {
        return Response.json(
          { error: "Camshot unavailable — enable motion detection in Fully Kiosk settings" },
          { status: 422 },
        );
      }
